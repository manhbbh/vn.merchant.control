<template>
   <div class="py-3 px-2 lg:px-4 bg-white rounded-lg flex text-black items-start gap-2 lg:gap-3">
      <!-- icon -->
      <IconPapers class="w-5 h-5 flex-shrink-0 hidden md:block"></IconPapers>
      <!--content  -->
      <div class="flex flex-col gap-2.5 w-full">
         <div class="flex items-start justify-between">
            <h4 class="flex justify-start text-sm font-medium">
               <IconPapers class="w-5 h-5 flex-shrink-0 block md:hidden mr-2"></IconPapers>
         ...
         <!--  -->
         <table class="min-w-full border-none bg-white rounded-lg z-9">
            <thead class="bg-slate-200 h-7 text-xs font-semibold sticky top-0 text-customDarkBlue flex-shrink-0 z-9">
               <tr class="h-7">
            ...
            <tbody>
               <tr
                  v-for="(holiday, index) in Object.keys(form_of_work?.setting_data || {})"
                  :key="index"
               ...
               >
                  ...
                  <td class="text-left font-medium py-2 hidden lg:table-cell">
                     ...
                     <div class="flex gap-1">
                        <p
                           v-for="(time, index) in form_of_work?.setting_data?.[holiday]?.working_time"
                           :class="{
                              'bg-green-500': Number(time?.working_hours || 0) >= 5 && time?.active,
                              'bg-orange-500': Number(time?.working_hours || 0) < 5 && time?.active,
                  ...
                  <td class="text-left py-2 text-customGray hidden lg:table-cell">
                     <div class="flex items-center gap-1 h-5">
                        <Avatar
                           ...
                           :text="getInfo(form_of_work?.setting_data?.[holiday]?.created_by, 'name') || ''"
                        ></Avatar>
                        <!--  -->
                        <div class="flex gap-1 items-end justify-between">
                           <p class="text-sm">
                              {{ getInfo(form_of_work?.setting_data?.[holiday]?.created_by, 'name') }}
                           </p>
                     ...
                     <div class="h5 flex ml-5 items-center justify-start text-xs">
                        <p v-if="form_of_work?.setting_data?.[holiday]?.created_time">
                           {{
                              format(form_of_work?.setting_data?.[holiday]?.created_time as Date, 'HH:mm - dd/MM/yyyy')
                           }}
                        </p>
                     </div>
                  </td>
                  ...
                  <td class="text-center py-2">
                     <div class="flex gap-1 lg:gap-2.5 justify-center">
                        <button
                           @click="showModal(form_of_work?.setting_data?.[holiday], holiday)"
   ...
   >
      <div class="flex flex-col w-full">
         ...
         <!--  -->
         <main class="max-h-[80dvh] flex flex-col py-2 lg:p-6 border-b border-t gap-2 border-gray-200 overflow-hidden">
            ...
            <div class="lg:pl-8 overflow-y-auto w-full px-3 lg:px-0">
               ...
               >
                  ...
                  >
                     <div class="hidden lg:flex items-center flex-shrink-0 gap-2 lg:gap-4">
                        <!-- giờ làm -->
...
function showModal(item?: FormOfWorkData[string], idx?: string) {
   ...
   // nếu thêm mới thì gán dữ liệu mặc định
      ...
      index.value = (Number(Object.keys(form_of_work.value.setting_data || {})?.pop()) + 1)?.toString()
   }
}
...
function handleDelete(id: string) {
   confirm('warning', t('v1.confirm.delete_form_of_work'), '', (is_cancel: boolean) => {
      if (is_cancel) return
...
function resetModal() {
   confirm('warning', t('v1.confirm.restore_default'), '', (is_cancel: boolean) => {
      if (is_cancel) return
      form_of_work_value.value.working_time = cloneDeep(setting.form_of_work['1']).working_time
   })
}
