<template>
   <div class="py-3 px-2 sm:px-4 bg-white rounded-lg flex text-black items-start gap-2 sm:gap-3">
      <!-- icon -->
      <IconPeriod class="w-5 h-5 flex-shrink-0 hidden md:block"></IconPeriod>
      <!--content  -->
      <div class="flex-col flex-1">
         <div class="flex flex-col items-start mb-3">
            <h4 class="flex justify-start text-sm font-medium">
               <IconPeriod class="w-5 h-5 flex-shrink-0 block md:hidden mr-2"></IconPeriod>
               {{ $t('v1.setting.business_period') }}
            </h4>
            <p class="text-sm text-left text-muted max-md:mt-2">
         ...
         <div class="flex flex-col gap-3 sm:flex-row">
            ...
            >
               <h3 class="text-sm font-medium">{{ $t('v1.setting.month') }}<span class="text-error"> *</span></h3>
               <div class="flex items-center border border-gray-300 h-9 py-2 rounded-md">
                  <p class="text-muted w-28">{{ $t('v1.setting.start_date') }}</p>
                  <div class="border-l w-12 border-gray-300 flex items-center justify-center h-9">
                     <select
               ...
               <div class="flex gap-4">
                  <div class="flex items-center border border-gray-300 rounded-md">
                     ...
                     <div
                        ...
                        :class="{
                           'bg-slate-100': monthly_business_period.setting_data.end === 32,
                        }"
                     >
                        <select
            ...
            <div class="flex flex-col gap-1 items-start w-80">
               <h3 class="text-sm font-medium">{{ $t('v1.setting.year') }}<span class="text-error"> *</span></h3>
               <!--  -->
               <div class="flex items-center border text-sm border-gray-300 h-9 py-2 rounded-md">
                  <p class="text-muted w-40">{{ $t('v1.setting.start_day_month') }}</p>
                  <div class="border-l w-14 border-gray-300 flex items-center justify-center h-9">
                     <CustomVuePicker
                        class=""
                        ...
                        :handle-date="
                           () => {
                              ...
                              year_business_period.setting_data.start = {
                                 ...
                                 month: start_date_year?.getMonth() && start_date_year?.getMonth() + 1,
                              }
                           }
                        "
                        :hidden_icon="true"
                        ...
                        :handle-date="
                           () => {
                              ...
                              year_business_period.setting_data.end = {
                                 ...
                                 month: end_date_year?.getMonth() && end_date_year?.getMonth() + 1,
                              }
                           }
                        "
                        :hidden_icon="true"
...
const { monthly_business_period, year_business_period } = storeToRefs(commonStore)
