<template>
   <div class="py-3 px-2 sm:px-4 bg-white rounded-lg flex text-black items-start gap-2 sm:gap-3">
      <!-- icon -->
      <IconCalendar class="w-5 h-5 flex-shrink-0 hidden md:block"></IconCalendar>
      <!--content  -->
      <div class="flex flex-col gap-2.5 w-full">
         <div class="flex items-start justify-between">
            <h4 class="flex justify-start text-sm font-medium">
               <IconCalendar class="w-5 h-5 flex-shrink-0 block md:hidden mr-2"></IconCalendar>
               {{ $t('v1.setting.holiday') }}
            </h4>
         ...
         <div class="grid grid-cols-3">
            <div class="flex gap-3 items-center w-full">
               ...
               <CustomSelectV3
                  ...
                  :default_value="{ value: holidays.calculate_holiday_pay }"
                  ...
                  :update="(item: HolidayPayOption) => {
                     holidays.calculate_holiday_pay = item.value
                  }"
         ...
         <table class="min-w-full border-none bg-white rounded-lg z-9">
            ...
            <tbody>
               <tr
                  v-for="(holiday, index) in Object.keys(holidays?.setting_data || {})"
                  :key="index"
                  ...
                  <td class="text-left py-2 text-customGray flex-col text-15px hidden md:flex">
                     <div class="flex items-center gap-1 h-5">
                        <Avatar
                           ...
                           :text="getInfo(holidays?.setting_data?.[holiday]?.created_by, 'name') || ''"
                        ></Avatar>
                        <!--  -->
                     ...
                     <div class="h5 flex ml-5 items-center justify-start text-xs">
                        <p v-if="holidays?.setting_data?.[holiday]?.created_time">
                           {{ format(holidays?.setting_data?.[holiday]?.created_time as Date, 'HH:mm - dd/MM/yyyy') }}
                        </p>
                     </div>
                  </td>
...
// * interfaces
import { CalculationMethod, HolidaySetting, HolidaySettingData } from '@/service/interface'
