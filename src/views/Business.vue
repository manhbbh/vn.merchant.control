      ...
      >
         ...
         >
            ...
            <div class="relative text-start group">
               ...
               <div class="hidden group-hover:block hover:block absolute top-9 py-1 -right-4 z-50">
                  <div class="flex flex-col border mr-2 rounded-lg bg-white shadow-lg">
         ...
         >
            ...
            >
               ...
               <ul
                  v-if="!is_business"
                  class="flex flex-col gap-3 ml-2 md:ml-8 pb-2 overflow-auto"
               >
                  <li
                  ...
                  >
                     ...
                     <div class="flex-1 flex flex-col justify-start overflow-hidden">
                        ...
                        <p class="text-slate-500 text-xs h-4 flex justify-start truncate">
                           {{ control.type === 'headquarter' ? $t('v1.common.headquarter') : $t('v1.common.branch') }}
                        </p>
                     </div>
                  </li>
               </ul>
...
import { useIsMobile } from '@/composable/mobile'
...
const { branches, branch_data, employees_user_ids, user, is_get_data, businesses } = storeToRefs(commonStore)
...
const branchDetailRef = ref<HTMLElement>()

/** biên lưu data có hiển thị header không */
const is_show_header = ref<boolean>(true)
...
onMounted(() => {
   ...
   if (route.query?.show_header === 'false' && isMobile.value) {
      is_show_header.value = false
   }
})
...
/** Nếu là màn mobile thì scroll xuống */
async function scrollToBranchDetailOnMobile() {
   if (!isMobile.value) return

   await nextTick()

   branchDetailRef.value?.scrollIntoView({
      behavior: 'smooth',
      block: 'start',
   })
}
